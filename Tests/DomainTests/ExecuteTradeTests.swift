...
/// ExecuteTrade(검증) + 실제 포트폴리오 math(MockLocalStore.recordTrade) 테스트.
/// 기대값은 손계산 독립 앵커(자기참조 금지). 수식 재구현 없이 실로직을 호출한다.
final class ExecuteTradeTests: XCTestCase {

    private func makeSystem(cash: Int = 1_000_000) -> (ExecuteTrade, MockPortfolioRepository) {
        let store = MockLocalStore(cash: cash)
        let repo = MockPortfolioRepository(store: store)
        return (ExecuteTrade(portfolio: repo), repo)
    }
    ...
    func testRejectsEmptyRationale() async {
        let (execute, _) = makeSystem()
        await assertThrowsValidation {
            _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_000), rationaleText: "   ")
        }
    }
    ...
    func testRejectsOver140CharRationale() async {
        ...
        await assertThrowsValidation {
            _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_000), rationaleText: long)
        }
    }
    ...
    func testRejectsZeroQuantity() async {
        let (execute, _) = makeSystem()
        await assertThrowsValidation {
            _ = try await execute(type: .buy, stockCode: "005930", quantity: 0, price: .krw(1_000), rationaleText: "근거")
        }
    }
    ...
    func testBuy_reducesCash_addsHolding() async throws {
        let (execute, repo) = makeSystem(cash: 1_000_000)
        // 앵커: 1000원 10주 매수 → cash −10,000, 보유 10주 평단 1000
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_000), rationaleText: "근거")
    ...
    func testMultipleBuys_weightedAveragePrice() async throws {
        ...
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_000), rationaleText: "근거")
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_200), rationaleText: "근거")
        let p = try await repo.fetchPortfolio()
    ...
    func testAveragePrice_truncatesFraction() async throws {
        ...
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_000), rationaleText: "근거")
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_233), rationaleText: "근거")
        let p = try await repo.fetchPortfolio()
    ...
    func testPartialSell_increasesCash_keepsAveragePrice() async throws {
        let (execute, repo) = makeSystem(cash: 1_000_000)
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_000), rationaleText: "근거")
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_200), rationaleText: "근거") // 20@1100
        let cashBefore = try await repo.fetchPortfolio().cash.amount
        // 앵커: 1300원 5주 매도 → cash +6,500, 보유 15주 평단 1100 유지
        _ = try await execute(type: .sell, stockCode: "005930", quantity: 5, price: .krw(1_300), rationaleText: "근거")
        let p = try await repo.fetchPortfolio()
    ...
    func testFullSell_removesHolding() async throws {
        let (execute, repo) = makeSystem(cash: 1_000_000)
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_000), rationaleText: "근거")
        _ = try await execute(type: .sell, stockCode: "005930", quantity: 10, price: .krw(1_100), rationaleText: "근거")
        let p = try await repo.fetchPortfolio()
    ...
    func testInsufficientCash_throwsValidation() async {
        let (execute, _) = makeSystem(cash: 5_000) // 1000*10 = 10,000 > 5,000
        await assertThrowsValidation {
            _ = try await execute(type: .buy, stockCode: "005930", quantity: 10, price: .krw(1_000), rationaleText: "근거")
        }
    }
    ...
    func testInsufficientHolding_throwsValidation() async throws {
        let (execute, _) = makeSystem(cash: 1_000_000)
        _ = try await execute(type: .buy, stockCode: "005930", quantity: 3, price: .krw(1_000), rationaleText: "근거")
        await assertThrowsValidation {
            _ = try await execute(type: .sell, stockCode: "005930", quantity: 5, price: .krw(1_000), rationaleText: "근거")
        }
    }
}
